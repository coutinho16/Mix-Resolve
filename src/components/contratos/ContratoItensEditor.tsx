...
import {
  ...
  removerItemContrato,
  salvarValorSetorContrato,
} from "@/app/gestao/contratos/actions";

interface ContratoItensEditorProps {
  contratoId: string;
...
export function ContratoItensEditor({
  contratoId,
  equipamentos,
  categorias,
  itens,
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      await adicionarItemContrato(contratoId, formData);
    ...
    <div className="flex flex-col gap-5">
      <div>
        ...
        <p className="text-xs text-neutro-1">
          Mesma lista de equipamentos do sistema, agrupada por setor no contrato.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-dashed border-neutro-2 bg-neutro-3/60 p-4">
        ...
        <form ref={formRef} onSubmit={onSubmit} className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex min-w-[220px] flex-1 flex-col gap-1.5">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  (form.elements.namedItem("descricao") as HTMLInputElement).value = eq?.nome ?? "";
          ...
          {origem === "manual" && (
            <div className="flex min-w-[220px] flex-1 flex-col gap-1.5">
              ...
              <input
                ...
                onChange={(e) => {
                  const form = e.target.form!;
                  (form.elements.namedItem("descricao") as HTMLInputElement).value = e.target.value;
                }}
                className="rounded-lg border border-neutro-2 bg-branco-puro px-3 py-2 text-sm outline-none focus:border-laranja"
              />
            </div>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold uppercase tracking-wide text-neutro-1">Qtd.</label>
            <input
              name="quantidade"
              type="number"
              min={1}
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-dashed border-neutro-2 p-4">
          ...
          {setoresComItens.map((setor) => {
            ...
            return (
              <form
                key={setor}
                action={(fd) => salvarValorSetorContrato(contratoId, fd)}
              ...
              <ul className="flex flex-col gap-1.5">
                {itensDoSetor.map((item) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => removerItemContrato(contratoId, item.id)}
                      aria-label="Remover"
